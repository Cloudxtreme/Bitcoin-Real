...
pkgdatadir = $(datadir)/bitcoinreal
pkgincludedir = $(includedir)/bitcoinreal
pkglibdir = $(libdir)/bitcoinreal
pkglibexecdir = $(libexecdir)/bitcoinreal
...
CONFIG_HEADER = $(top_builddir)/src/config/bitcoinreal-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/bitcoinreal-config.h.in \
...
ACLOCAL = ${SHELL} /root/Bitcoin-Real/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /root/Bitcoin-Real/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /root/Bitcoin-Real/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /root/Bitcoin-Real/build-aux/missing automake-1.15
AWK = gawk
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = bitcoinreal-cli
BITCOIN_DAEMON_NAME = bitcoinreald
BITCOIN_GUI_NAME = bitcoinreal-qt
BITCOIN_TX_NAME = bitcoinreal-tx
...
LUPDATE = 
MAINT = 
MAKEINFO = ${SHELL} /root/Bitcoin-Real/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = bitcoinreal
PACKAGE_BUGREPORT = www.bitcoinreal.io
PACKAGE_NAME = BitcoinReal Core
PACKAGE_STRING = BitcoinReal Core 4.1.1
PACKAGE_TARNAME = bitcoinreal
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /root/Bitcoin-Real
abs_srcdir = /root/Bitcoin-Real
abs_top_builddir = /root/Bitcoin-Real
abs_top_srcdir = /root/Bitcoin-Real
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /root/Bitcoin-Real/build-aux/install-sh
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_bitcoinreal_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info \
  leveldb_baseline_filtered.info test_bitcoinreal_coverage.info test_bitcoinreal.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/bitcoinreal-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/bitcoinreal-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/bitcoinreal-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/bitcoinreal-config.h
$(top_srcdir)/src/config/bitcoinreal-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/bitcoinreal-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/bitcoinreal-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/bitcoinreal-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/bitcoinreal-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_bitcoinreal.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_bitcoinreal -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_bitcoinreal_filtered.info: test_bitcoinreal.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test_filtered.info: block_test.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#test_bitcoinreal_coverage.info: baseline_filtered_combined.info test_bitcoinreal_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_bitcoinreal_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_bitcoinreal_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_bitcoinreal_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_bitcoinreal.coverage/.dirstamp:  test_bitcoinreal_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_bitcoinreal.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf test_bitcoinreal.coverage/ total.coverage/ $(OSX_APP)
