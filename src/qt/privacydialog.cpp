...
#include "zcatocoincontroldialog.h"
#include "spork.h"
...
void PrivacyDialog::on_pushButtonZCatocoinControl_clicked()
{
    ZCatocoinControlDialog* zCatocoinControl = new ZCatocoinControlDialog(this);
    zCatocoinControl->setModel(walletModel);
    zCatocoinControl->exec();
}

void PrivacyDialog::setZCatocoinControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzCatocoinSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzBR()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Catocoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    // use mints from zCatocoin selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZCatocoinControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZCatocoinControlDialog::GetSelectedMints();
    }
    ...
    // Clear zcatocoin selector in case it was used
    ZCatocoinControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zCatocoin Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Catocoin, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zCatocoin Mint");
